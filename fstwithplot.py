...
def extract_and_makelist(accept_dataframe, **kwargs):
...
def calculate_fst(accept_dataframe, **kwargs):

  ########  call  extract function######### 
  dict_allelecounts = extract_and_makelist(accept_dataframe, **kwargs)
...
def make_df(accept_dataframe, **kwargs):
  fst_stores = calculate_fst(accept_dataframe, **kwargs) # get fst store dict
...
#make_df(complete_df, pop1 = 'GBR', pop2 = 'MXL', pop3 = 'CDX', pop4 = 'GIH', pop5 = 'LWK')

#################################################################
######make heatplot from fst dataframe ##########################
#################################################################

def make_heatplot(accept_dataframe, **kwargs):
  
  fst_df = make_df(complete_df,  **kwargs) # get fst_df
  
   #### size of figure ####
  figure, ax = plt.subplots(figsize = (10,5))
  fst_df = fst_df.apply(pd.to_numeric) # convert each col in df to float from object for seaborn plot
  fig=sb.heatmap(fst_df, cmap = 'RdYlGn', annot = True) 
  heatmap = fig.get_figure()
  heatmap.savefig('test.png')
  
make_heatplot(complete_df, pop1 = 'GBR', pop2 = 'MXL' , pop3 = 'CDX', pop4 = 'LWK')
