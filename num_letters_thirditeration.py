...
def make_dataframe(function,file):
    alleles=extract_fields(lines,file)
    allelesdf= pandas.DataFrame.from_dict(alleles)
    return allelesdf
...
def make_file(function,file):
  dfalelle= make_dataframe(extract_fields,file)
  
  return dfalelle.to_csv("gt.tsv",sep='\t')
...
make_dataframe(extract_fields,'filtered_GBR_biallelic.csv')
make_file(make_dataframe, 'filtered_GBR_biallelic.csv')